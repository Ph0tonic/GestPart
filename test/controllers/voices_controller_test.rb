...
class VoicesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create voice" do
    assert_difference('Voice.count') do
      post voices_url, params: { voice: { name: @voice.name, number: @voice.number, order: @voice.order } }
    end
  ...
  test "should update voice" do
    patch voice_url(@voice), params: { voice: { name: @voice.name, number: @voice.number, order: @voice.order } }
